...
package object typingrule {
  ...
  def boundedMultiplicityType[T <: BaseType : ClassTag](e: SExp, upperBound: MultiplicityBounds)(implicit typeSystem: TypeSystem[SExp, Type]) = multiplicityType[T](e).flatMap{t => upperBounded(e, t, upperBound)}
  ...
  def lub(o: HasOrigin, t1: MultiplicityType[BaseType], t2: MultiplicityType[BaseType])(implicit typeSystem: TypeSystem[SExp, Type], ord: Ordering[NumericType]): TypingRule.Aux[SExp, Type, MultiplicityType[BaseType]] = {
    for {
      baseType <- (t1.baseType, t2.baseType) match{
        case (n1: NumericType, n2: NumericType) => rule.success[BaseType](ord.max(n1, n2))
        case (b1, b2) if(b1 == b2) => rule.success[BaseType](b1)
        case (b1, b2) => rule.fail[BaseType](o, s"incompatible base types: ${Type.ppBaseType(b1)} != ${Type.ppBaseType(b2)}")
      }
  ...
  def lubMultiplicity(o: HasOrigin, m1: MultiplicityBounds, m2: MultiplicityBounds)(implicit typeSystem: TypeSystem[SExp, Type]): TermTypingRule[SExp, Type, MultiplicityBounds] =
    (m1.tryCompare(m2) match{
      case Some(i) => rule(typeSystem).success(if(i > 0) m2 else m1)
      case None => rule.fail[MultiplicityBounds](o, s"incompatible multiplicity bounds: $m1 <-> $m2")
    }).bindTerm(o)

  def maybeEmpty(e: SExp)(implicit typeSystem: TypeSystem[SExp, Type]): TermTypingRule[SExp, Type, MultiplicityType[BaseType]] =
    multiplicityType[BaseType](e).filter(_.multiplicity.lowerBound == Multiplicity.zero , t => "type should be able to inhabit no instances")
